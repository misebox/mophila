...
エスケープタイム系フラクタル (Mandelbrot、Julia、Burning Ship、Multibrot) は `lib/fractal.moph` を使う: `import { escape_time, mandelbrot, julia, perturbation, plain, smooth } from ..lib.fractal` して `escape_time(formula:, precision:, coloring:, center:, span:, zoom:, max_iter:, samples:)` が Shader を返す。反復式は Dict (seed / step / start / delta / degree)、色付けは `func (mu, z) -> Color` で、どちらも自作できる。深く寄るなら `perturbation` (基準軌道をスクリプト側の 64 bit で計算し GPU は差分だけ。10 兆倍あたりまで)。samples/mandelbrot.moph、julia.moph、burning_ship.moph が例。
