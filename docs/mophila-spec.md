...
`--trim` が時間の切り取りで、`--crop` が領域の切り取り。`--trim` の時刻は `00:15` `1.5s` `500ms` の他に、`94%` と割合でも、
`10978` とコマ数でも書ける (単位も `:` も無ければコマ数。`--fps` で秒に直す)。
`94%..96%` や `2s..50%` のように混ぜてもよい。`--crop height` は高さを箱いっぱいにして幅を出力の比から決め、
