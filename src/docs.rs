...
pub const TYPES: &[Type] = &[
    ...
    Type {
        ...
        values: &[],
        members: &[],
        doc: "Number だけを詰めた配列。要素ごとの値を持たないので、Rust で作った大きな並びを Rust の関数や Shader.args にそのまま渡せる (Shader.args は同じ Array である間は GPU に送り直さない)。作ったら変えない (push は無い)。添字、範囲、for、len、enumerate は List と同じ",
    },
    Type {
        name: "Dict",
        category: "Collection",
    ...
    Type {
        ...
        category: "Paint",
        union: "",
        make: "ZoomPath(center = Vector(8, 4.5), zoom = func (t) { t + 1 }, duration = 2m)",
        values: &[],
        members: &[],
    ...
    Type {
        ...
        values: &[],
        members: &[],
        doc: "どの対象のどの属性が、いつ、どの値になるかの並び。行に属性への代入を書いた motion がこれになる。別の Timeline、Audio、Narration も place で時刻に置ける",
    },
    Type {
        name: "Motion",
        category: "Time",
    ...
    Type {
        ...
        values: &[],
        members: &[],
        doc: "読み上げ。Timeline に置くと、その文を読んだ音声が動画の音になり、同じ文が字幕にもなる。画面には描かない",
    },
    Type {
        name: "Anchor",
        category: "Enum",
...
pub const ATTRS: &[(&str, &str, &str)] = &[
    ...
    ("Shader", "args", "Number の Array か List。color の 4 つ目の引数。毎フレーム読む。Array なら同じものである間は GPU に送り直さない"),
    ("Shader", "samples", "1 ピクセルあたりの評価点の数 (4 なら 2x2 の平均。省略は 1)"),
    ("Shader", "zoom", "ズーム動画の道筋。入れると color の引数が (中心からの距離の対数, 角度, 時刻) になる"),
    ("ZoomPath", "center", "箱の座標での、寄っていく先"),
    ("ZoomPath", "zoom", "倍率の時間変化。func (t) -> Number。t は秒。増えていくこと (戻ると帯を作り直すので遅い)"),
    ("ZoomPath", "duration", "zoom を読む範囲。動画の長さ"),
    ("ZoomPath", "unit", "倍率 1 のときの、箱の座標 1 あたりのシェーダの座標の長さ (省略は 1)"),
    ("ZoomPath", "scale", "zoom と duration と unit から作った表。書くものではない"),
